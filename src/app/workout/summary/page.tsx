    ...
    <div className="flex flex-col min-h-screen bg-background">
      <main className="flex-1 p-6 space-y-8 pb-32">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {allRecords.map((rec, i) => (
                <div key={i} className="bg-primary/5 border border-primary/20 rounded-2xl p-4 flex items-center justify-between">
        ...
        <div className="flex flex-col gap-3 pt-6 pb-20">
          <Link href="/">
            <Button className="w-full h-16 rounded-[2rem] font-black uppercase tracking-widest text-lg shadow-2xl shadow-primary/30">Done</Button>
          </Link>
          <Button variant="ghost" className="w-full h-14 rounded-2xl font-black uppercase tracking-widest text-sm text-muted-foreground"><Share2 className="h-4 w-4 mr-2" /> Share Summary</Button>
        </div>
      </main>
    </div>
  );
